...
# Register `Sequential` secara manual
keras.utils.get_custom_objects().update({'Sequential': keras.models.Sequential})
        ...
        if not success:
            ...
            for (x,y,w,h) in faces_detected:  
                   ...
                   "Angry":(0,0,255), "Disgust": (230,159,0), "Neutral": (0,255,255), "Fear": (128,0,128)} 
                    ...
                    Text = f"{predicted_emotion} ({predictions[0][np.argmax(predictions[0])]:.2%})"   
...
def Emotion_Analysis(img):
    ...
    gray_frame = cv2.cvtColor(imge, cv2.COLOR_BGR2GRAY)
    ...
    for (x, y, w, h) in faces:
        ...
        ## Defining the Parameters for putting Text on Image
        Text = f"{pred_emotion} ({prediction[0][np.argmax(prediction[0])]:.2%})"
        
        cv2.rectangle(imge,(x,y),(x+w,y+h),rec_col[str(pred_emotion)],2)
        cv2.rectangle(imge,(x,y-40),(x+w,y),rec_col[str(pred_emotion)],-1)
        cv2.putText(imge, Text, (x, y-10),cv2.FONT_HERSHEY_SIMPLEX,0.6,(0,0,0),2)
